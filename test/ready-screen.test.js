...
test("compact Ready rules cover 568x320 and 667x375 without hiding labels", () => {
  ...
  assert.match(compactCss, /\.ready-screen \{[\s\S]*?max\(16px, var\(--safe-inline-end\)\)[\s\S]*?max\(28px, var\(--safe-inline-start\)\)/);
  assert.match(compactCss, /\.ready-top \{\s*gap: 8px;/);
  assert.match(compactCss, /\.ready-bottom \{[\s\S]*?gap: 8px;/);
